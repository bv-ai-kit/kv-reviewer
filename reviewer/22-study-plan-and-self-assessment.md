...
```mermaid
flowchart LR
    New[Study module] --> R1[Recall same day]
    R1 --> R2[Recall plus 2 days]
    R2 --> R3[Recall plus 1 week]
    R3 --> R4[Recall plus 2-3 weeks]
    R4 --> Stick{Sticks?}
    Stick -- Yes --> Done[Retire card]
...
```mermaid
flowchart TD
    Charter["1. Charter (why and what)"] --> WBS["2. WBS (the work)"]
    WBS --> Sched["3. Schedule and critical path"]
    Charter --> Stake["5. Stakeholder grid"]
    WBS --> Risk["4. Risk register"]
    Sched --> Status["6. One-page status report"]
    Risk --> Status
    Stake --> Status
```
...
```mermaid
flowchart TD
    Start([Ready to interview?]) --> Q1{Most areas at 4+?}
    Q1 -- No --> Study[Target weakest rows this week]
    Study --> Start
    Q1 -- Yes --> Q2{Capstone artifacts built?}
    Q2 -- No --> Cap[Finish the capstone]
    Cap --> Start
    Q2 -- Yes --> Q3{EVM and Scrum cold?}
    Q3 -- No --> Drill[Drill leech list]
    Drill --> Start
