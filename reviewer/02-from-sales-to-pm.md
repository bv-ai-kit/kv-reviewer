...
```mermaid
flowchart LR
    subgraph Sales["What you do now"]
        S1["Pipeline management"]
        S2["Objection handling"]
        S3["Discovery calls"]
        S4["Quota and targets"]
        S5["CRM"]
        S6["Forecasting"]
        S7["Closing"]
        S8["Account management"]
        S9["Negotiation"]
    end
    subgraph PM["What it becomes"]
        P1["Project tracking"]
        P2["Risk and stakeholder mgmt"]
        P3["Requirements gathering"]
        P4["Scope and success criteria"]
        P5["PM tools"]
        P6["Estimation"]
        P7["Delivery and sign-off"]
        P8["Stakeholder engagement"]
        P9["Procurement and conflict"]
    end
    S1 --> P1
    S2 --> P2
    S3 --> P3
...
```mermaid
mindmap
  root((Your transferable strengths))
    Relationships
      Reading the room
      Building trust fast
      Managing up
...
```mermaid
quadrantChart
    title Skills you already have vs skills to build
    x-axis "Low confidence" --> "High confidence"
    y-axis "Lower priority" --> "Higher priority"
    quadrant-1 "Lean on these"
    quadrant-2 "Close these fast"
    quadrant-3 "Build over time"
    quadrant-4 "Quiet strengths"
    "Stakeholder comms": [0.85, 0.9]
    "Requirements discovery": [0.8, 0.88]
    "Driving to sign-off": [0.82, 0.75]
    "Resilience under pressure": [0.9, 0.6]
    "WBS and scheduling": [0.25, 0.85]
    "Methodology fluency": [0.3, 0.8]
    "PM tooling": [0.35, 0.65]
