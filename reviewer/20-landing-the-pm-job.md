...
```mermaid
flowchart LR
    A["Sales bullet: 'Closed a $250K deal'"] --> B["Find the hidden project"]
    B --> C["Name the moving parts and stakeholders"]
    C --> D["Add a PM verb: Led / Coordinated"]
...
```mermaid
flowchart TD
    S["Your sales background"] --> O["On-ramp roles"]
    O --> C1["Project Coordinator"]
    O --> C2["Associate / Junior PM"]
    O --> C3["Scrum Master"]
    O --> C4["Implementation / Onboarding Manager"]
    C1 --> M["Mid-level PM in 1-2 years"]
    C2 --> M
    C3 --> M
    C4 --> M
...
```mermaid
flowchart TD
    P["1.0 Acme Rollout"] --> A["1.1 Initiation"]
    P --> B["1.2 Data Migration"]
    P --> C["1.3 Training"]
    P --> D["1.4 Go-Live"]
    A --> A1["1.1.1 Kickoff meeting"]
    A --> A2["1.1.2 Charter signed"]
    B --> B1["1.2.1 Export legacy data"]
    B --> B2["1.2.2 Map and import"]
    B --> B3["1.2.3 Validate"]
    C --> C1["1.3.1 Build materials"]
    C --> C2["1.3.2 Run sessions"]
    D --> D1["1.4.1 Cutover"]
...
```mermaid
flowchart LR
    S["S — Situation: set the scene briefly"] --> T["T — Task: your specific responsibility"]
    T --> A["A — Action: what YOU did, step by step"]
    A --> R["R — Result: the quantified outcome"]
```
...
```mermaid
journey
    title Your interview-prep journey
    section Research
      Study the company and role: 4: You
      Map JD keywords to your stories: 3: You
    section Prepare
      Write 5-6 STAR stories: 4: You
      Rehearse out loud: 3: You
    section Interview
      Answer with STAR: 4: You
      Ask sharp questions: 5: You
    section Close
...
timeline
    ...
    Days 1-30 : Meet stakeholders : Learn tools and process : Read project history
    Days 31-60 : Own status reports : Run check-ins : Surface risks early
